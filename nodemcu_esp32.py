...
class NodeMcuESP32S(Board):
    ...
    def burn(self,bin,outfn=None):
        bauds = [115200*x for x in range(8,0,-1)]
        ...
        for bb in bauds:
            res,out,err = proc.runcmd("python",tools["esptool32"],"--chip", "esp32","--port",self.port,"--baud",str(bb),"--before", "default_reset", "--after", "hard_reset","write_flash","-z","--flash_freq","40m","--flash_mode","dio","--flash_size","detect","0x1000",romf, "0x10000",iromf, "0x8000", bromf,"0x390000",vromf,outfn=outfn)
            if res:
                continue
            else:
                break
